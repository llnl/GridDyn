...
if ((filecount > 0 && filecount <= 20)); then
    ...
    cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DGRIDDYN_BUILD_C_SHARED_LIBRARY=ON -DGRIDDYN_ENABLE_FMI_EXPORT=ON -DGRIDDYN_ENABLE_NETWORKING_LIBRARY=ON -DGRIDDYN_ENABLE_SERVER_EXECUTABLE=ON -DGRIDDYN_ENABLE_TCP=ON -DGRIDDYN_ENABLE_DIME=ON -DGRIDDYN_ENABLE_ZMQ=ON ..
    TIDY_EXTRA_ARGS=()
