...
# set default verbosity Process the CMake automatically-generated var: SuiteSparse_FIND_QUIETLY:
# supersedes *_VERBOSE.
...
# set the LIB POSTFIX to find in right directory according to what kind of compiler we use
# (32/64bits) July 2017 git commit 1618fd1 made so only Linux/BSD/GNU installs to lib/lib64, others
# use lib; this remains to avoid breaking older installs
...
if(SuiteSparse_SEARCH_LIB_POSTFIX)
    ...
    if(SuiteSparse_VERBOSE)
        message(
            STATUS
                "   find_library will search inside lib${SuiteSparse_SEARCH_LIB_POSTFIX} directory (can be changed with SuiteSparse_SEARCH_LIB_POSTFIX)"
        )
    endif()
endif()
...
# This utility macro is used to find all suitesparse projects by giving its name Since the name
# structure is the same for lib name and include dir name, we can use a generic way to find all of
# these with simple cmake lines of code
macro(SuiteSparse_FIND_COMPONENTS)
    ...
    # special check for suitesparse component (allow to find on windows but not on linux because
    # doesn't exist)
    list(FIND SuiteSparse_FIND_COMPONENTS "suitesparse" ss_index)
    if(${ss_index} MATCHES "-1")
        # do nothing, the user didn't provide the suitesparse component
    else()
        if(WIN32)
            # do nothing, the user provide the suitesparse component we will try to find
        else()
            list(REMOVE_AT SuiteSparse_FIND_COMPONENTS ${ss_index})
            if(SuiteSparse_VERBOSE)
                message(
                    STATUS
                        "   On this platform : suitesparse lib doesn't exist (only on windows), so skip this component"
                )
            endif()
        endif()
    endif()

    # Look for each component the same way : * For include dir the reference file is the
    # <component>.h * for library fileName the reference is the <component> itself (cmake will
    # prepend/append necessary prefix/suffix according to the platform)
    foreach(suitesparseComp ${SuiteSparse_FIND_COMPONENTS})

        # used to construct specific cmake variables (in upper case) according to the component, but
        # also used for find_*()
        ...
        # Special case: suitesparseconfig library is named "libsuitesparseconfig.*" but headers are
        ...
        find_path(
            ...
            NAMES ${suitesparseComp}.h
                  ${suitesparseCompLC}.h
                  ${suitesparseCompUC}.h
                  ${suitesparseComp_ALT}.h
                  ${suitesparseComp}.hpp
                  ${suitesparseCompLC}.hpp
                  ${suitesparseCompUC}.hpp
        ...
        if(SuiteSparse_${suitesparseCompUC}_INCLUDE_DIR)
            if(NOT ${SuiteSparse_${suitesparseCompUC}_INCLUDE_DIR} MATCHES
               ^${SuiteSparse_DIR_INCLUDE}.*$
            )
                if(NOT ${SuiteSparse_${suitespraseCompUC}_INCLUDE_DIR} MATCHES
                   ^${SuiteSparse_DIR}.*$
                )
                    if(NOT ${SuiteSparse_${suitespraseCompUC}_INCLUDE_DIR} MATCHES
                       ^${${suitesparseCompUC}_DIR}.*$
        ...
        find_library(
            SuiteSparse_${suitesparseCompUC}_LIBRARY_RELEASE
            NAMES lib${suitesparseComp}
                  lib${suitesparseCompLC}
                  lib${suitesparseCompUC}
                  ${suitesparseComp}
                  ${suitesparseCompLC}
                  ${suitesparseCompUC}
        ...
        find_library(
            SuiteSparse_${suitesparseCompUC}_LIBRARY_DEBUG
            NAMES ${suitesparseComp}d
                  ${suitesparseCompLC}d
                  ${suitesparseCompUC}d
                  lib${suitesparseComp}d
                  lib${suitesparseCompLC}d
                  lib${suitesparseCompUC}d
        ...
        if(SuiteSparse_${suitesparseCompUC}_LIBRARY_RELEASE)
            if(NOT SuiteSparse_${suitesparseCompUC}_LIBRARY_DEBUG)
                set(SuiteSparse_${suitesparseCompUC}_LIBRARY_DEBUG
                    ${SuiteSparse_${suitesparseCompUC}_LIBRARY_RELEASE}
                    CACHE PATH "Path to a library." FORCE
                )
            endif()
        endif()
        if(SuiteSparse_${suitesparseCompUC}_LIBRARY_DEBUG)
            if(NOT SuiteSparse_${suitesparseCompUC}_LIBRARY_RELEASE)
                set(SuiteSparse_${suitesparseCompUC}_LIBRARY_RELEASE
                    ${SuiteSparse_${suitesparseCompUC}_LIBRARY_DEBUG}
                    CACHE PATH "Path to a library." FORCE
                )
            endif()
        ...
        if(SuiteSparse_${suitesparseCompUC}_LIBRARY_RELEASE)
            if(NOT ${SuiteSparse_${suitesparseCompUC}_LIBRARY_RELEASE} MATCHES
               ^${SuiteSparse_DIR_LIB}.*$
            )
                if(NOT ${SuiteSparse_${suitespraseCompUC}_LIBRARY_RELEASE} MATCHES
                   ^${SuiteSparse_DIR}.*$
                )
                    if(NOT ${SuiteSparse_${suitespraseCompUC}_LIBRARY_RELEASE} MATCHES
                       ^${${suitesparseCompUC}_DIR}.*$
        ...
        if(SuiteSparse_${suitesparseCompUC}_LIBRARY_DEBUG)
            if(NOT ${SuiteSparse_${suitesparseCompUC}_LIBRARY_DEBUG} MATCHES
               ^${SuiteSparse_DIR_LIB_DEBUG}.*$
            )
                if(NOT ${SuiteSparse_${suitesparseCompUC}_LIBRARY_DEBUG} MATCHES
                   ^${SuiteSparse_DIR_LIB}.*$
                )
                    if(NOT ${SuiteSparse_${suitespraseCompUC}_LIBRARY_DEBUG} MATCHES
                       ^${SuiteSparse_DIR}.*$
                    )
                        if(NOT ${SuiteSparse_${suitespraseCompUC}_LIBRARY_DEBUG} MATCHES
                           ^${${suitesparseCompUC}_DIR}.*$
        ...
        # check and append the and SuiteSparse_LIBRARIES list, and warn if not found (release and
        # debug) otherwise
        if(NOT SuiteSparse_${suitesparseCompUC}_LIBRARY_RELEASE
           AND NOT SuiteSparse_${suitesparseCompUC}_LIBRARY_DEBUG
        )
            if(SuiteSparse_VERBOSE)
                message(
                    WARNING
            ...
            if(TARGET ${SuiteSparseNameSpace}::${suitesparseCompLC})
                ...
                if(SuiteSparse_${suitesparseCompUC}_LIBRARY_RELEASE
                   AND SuiteSparse_${suitesparseCompUC}_LIBRARY_DEBUG
                )
                    set_property(
                        TARGET ${SuiteSparseNameSpace}::${suitesparseCompLC} APPEND
        ...
        # here we allow to find at least the include OR the lib dir and just warn if one of both
        # missing
        if(NOT SuiteSparse_${suitesparseCompUC}_INCLUDE_DIR
           AND NOT SuiteSparse_${suitesparseCompUC}_LIBRARY_RELEASE
        )
        ...
        # for the search. Otherwise we don't need anymore to expose all intermediate variables
        if(NOT SuiteSparse_${suitesparseCompUC}_FOUND)
            set(SuiteSparse_${suitesparseCompUC}_DIR "$ENV{SuiteSparse_${suitesparseCompUC}_DIR}"
                CACHE PATH "${suitesparseComp} root directory"
            )
        else()
        ...
        if(SuiteSparse_VERBOSE)
            message(
                STATUS
                    "   SuiteSparse_${suitesparseCompUC}_FOUND = ${SuiteSparse_${suitesparseCompUC}_FOUND} : "
            )
            message(
                STATUS
                    "      * SuiteSparse_${suitesparseCompUC}_INCLUDE_DIR = ${SuiteSparse_${suitesparseCompUC}_INCLUDE_DIR}"
            )
            message(
                STATUS
                    "      * SuiteSparse_${suitesparseCompUC}_LIBRARY_DEBUG = ${SuiteSparse_${suitesparseCompUC}_LIBRARY_DEBUG}"
            )
            message(
                STATUS
                    "      * SuiteSparse_${suitesparseCompUC}_LIBRARY_RELEASE = ${SuiteSparse_${suitesparseCompUC}_LIBRARY_RELEASE}"
            )
        endif()
        ...
        if(NOT ${suitesparseComp} MATCHES "metis")
            ...
            if(SuiteSparse_VERBOSE)
                message(
                    STATUS
                        "      * SuiteSparse_${suitesparseCompUC}_DEFINITIONS = ${SuiteSparse_${suitesparseCompUC}_DEFINITIONS}"
                )
            endif()
            if(TARGET ${SuiteSparseNameSpace}::${suitesparseCompLC})
                set_target_properties(
                    ${SuiteSparseNameSpace}::${suitesparseCompLC}
                    PROPERTIES INTERFACE_COMPILE_DEFINITIONS "NPARTITION"
                )
            endif()
        endif()
...
if(NOT SuiteSparse_FIND_COMPONENTS)
    list(
        ...
        CXSPARSE
        UMFPACK
    ) # suitesparse and metis are not searched by default (special case)
endif()
...
if(SuiteSparse_USE_LAPACK_BLAS)
    ...
    if(NOT SuiteSparse_BLAS_LIBRARY)
        if(SuiteSparse_VERBOSE)
            # Send all msgs as "STATUS": We'll send an error at the bottom, only if "REQUIRED" is
            # set.
            message(
                STATUS
                    "   Failed to find SuiteSparse_BLAS_LIBRARY. Set it manually or set the SuiteSparse_BLAS_DIR to look for it inside."
            )
        endif()
        ...
        set_target_properties(
            ${SuiteSparseNameSpace}::blas PROPERTIES IMPORTED_LOCATION
                                                     "${SuiteSparse_BLAS_LIBRARY}"
        )
        if(TARGET blas)
            ...
            set_target_properties(blas PROPERTIES IMPORTED_LOCATION "${SuiteSparse_BLAS_LIBRARY}")
        endif()
    endif()
    ...
    if(NOT SuiteSparse_LAPACK_LIBRARY)
        if(SuiteSparse_VERBOSE)
            ...
            message(
                STATUS
                    "   Failed to find SuiteSparse_LAPACK_LIBRARY. Set it manually or set the SuiteSparse_LAPACK_DIR to look for it inside."
            )
        endif()
        set(SuiteSparse_LAPACK_DIR "$ENV{SuiteSparse_LAPACK_DIR}" CACHE PATH
        ...
        set_target_properties(
            ${SuiteSparseNameSpace}::lapack PROPERTIES IMPORTED_LOCATION
                                                       "${SuiteSparse_LAPACK_LIBRARY}"
        )
        if(TARGET lapack)
            ...
            set_target_properties(
                lapack PROPERTIES IMPORTED_LOCATION "${SuiteSparse_LAPACK_LIBRARY}"
            )
        endif()
    endif()
    ...
    if(WIN32)
        ...
        foreach(dllPattern ${dllPatternFileList})
            ...
            foreach(searchDir ${SuiteSparse_DLL_SEARCH_DIRS})
                file(GLOB SuiteSparse_DLL_${dllPatternUC} "${searchDir}/${dllPattern}*.dll"
                )# append the *.dll
...
if(SuiteSparse_LIBRARIES)
    # This approach doesn't work because the list contains debug information Linker gets messed up,
    # since 1st lib will only be used in optimized builds, 2nd lib will only be used in debug builds
...
if(NOT SuiteSparse_FOUND)
    ...
    if(NOT SuiteSparse_FIND_QUIETLY)
        if(SuiteSparse_FIND_REQUIRED)
            message(FATAL_ERROR "SuiteSparse required but some headers or libs not found.")
        else()
...
if(SuiteSparse_FOUND)
    if(TARGET ${SuiteSparseNameSpace}::amd)
        if(TARGET ${SuiteSparseNameSpace}::suitesparseconfig)
            set_target_properties(
                ${SuiteSparseNameSpace}::amd
                PROPERTIES INTERFACE_LINK_LIBRARIES "${SuiteSparseNameSpace}::suitesparseconfig"
            )
        endif()
    endif()
    ...
    if(TARGET ${SuiteSparseNameSpace}::cholamd)
        if(TARGET ${SuiteSparseNameSpace}::suitesparseconfig)
            set_target_properties(
                ${SuiteSparseNameSpace}::cholamd
                PROPERTIES
                    INTERFACE_LINK_LIBRARIES
                    "${SuiteSparseNameSpace}::amd;${SuiteSparseNameSpace}::camd;${SuiteSparseNameSpace}::colamd;${SuiteSparseNameSpace}::ccolamd;${SuiteSparseNameSpace}::suitesparseconfig;blas;lapack"
            )
        else()
            set_target_properties(
                ${SuiteSparseNameSpace}::cholamd
                PROPERTIES
                    INTERFACE_LINK_LIBRARIES
                    "${SuiteSparseNameSpace}::amd;${SuiteSparseNameSpace}::camd;${SuiteSparseNameSpace}::colamd;${SuiteSparseNameSpace}::ccolamd;blas;lapack"
            )
        endif()
    endif()
    ...
    if(TARGET ${SuiteSparseNameSpace}::ldl)
        if(TARGET ${SuiteSparseNameSpace}::suitesparseconfig)
            set_target_properties(
                ${SuiteSparseNameSpace}::ldl
                PROPERTIES INTERFACE_LINK_LIBRARIES "S${SuiteSparseNameSpace}::suitesparseconfig"
            )
        endif()
    endif()
    ...
    if(TARGET ${SuiteSparseNameSpace}::klu)
        if(TARGET ${SuiteSparseNameSpace}::suitesparseconfig)
            set_target_properties(
                ${SuiteSparseNameSpace}::klu
                PROPERTIES
                    INTERFACE_LINK_LIBRARIES
                    "${SuiteSparseNameSpace}::amd;${SuiteSparseNameSpace}::colamd;${SuiteSparseNameSpace}::btf;${SuiteSparseNameSpace}::suitesparseconfig"
            )
        else()
            set_target_properties(
                ${SuiteSparseNameSpace}::klu
                PROPERTIES
                    INTERFACE_LINK_LIBRARIES
                    "${SuiteSparseNameSpace}::amd;${SuiteSparseNameSpace}::colamd;${SuiteSparseNameSpace}::btf"
            )
        endif()
    endif()
    ...
    if(TARGET ${SuiteSparseNameSpace}::umfpack)
        if(TARGET ${SuiteSparseNameSpace}::suitesparseconfig)
            set_target_properties(
                ${SuiteSparseNameSpace}::umfpack
                PROPERTIES
                    INTERFACE_LINK_LIBRARIES
                    "${SuiteSparseNameSpace}::amd;${SuiteSparseNameSpace}::camd;${SuiteSparseNameSpace}::colamd;${SuiteSparseNameSpace}::ccolamd;${SuiteSparseNameSpace}::cholmod;${SuiteSparseNameSpace}::suitesparseconfig;blas;lapack"
            )
        else()
            set_target_properties(
                ${SuiteSparseNameSpace}::umfpack
                PROPERTIES
                    INTERFACE_LINK_LIBRARIES
                    "${SuiteSparseNameSpace}::amd;${SuiteSparseNameSpace}::camd;${SuiteSparseNameSpace}::colamd;${SuiteSparseNameSpace}::ccolamd;${SuiteSparseNameSpace}::cholmod;blas;lapack"
            )
        endif()
    endif()
    ...
    if(TARGET ${SuiteSparseNameSpace}::spqr)
        if(TARGET ${SuiteSparseNameSpace}::suitesparseconfig)
            set_target_properties(
                ${SuiteSparseNameSpace}::spqr
                PROPERTIES
                    INTERFACE_LINK_LIBRARIES
                    "${SuiteSparseNameSpace}::camd;${SuiteSparseNameSpace}::ccolamd;${SuiteSparseNameSpace}::cholmod;${SuiteSparseNameSpace}::suitesparseconfig;blas;lapack"
            )
        else()
            set_target_properties(
                ${SuiteSparseNameSpace}::spqr
                PROPERTIES
                    INTERFACE_LINK_LIBRARIES
                    "${SuiteSparseNameSpace}::camd;${SuiteSparseNameSpace}::ccolamd;${SuiteSparseNameSpace}::cholmod;blas;lapack"
            )
        endif()
    endif()
