...
ExternalProject_Add(minizip
    ...
    if(MSVC)
        ...
        if(NOT BUILD_RELEASE_ONLY)
            message(
                STATUS
                    "Configuring Minizip Autobuild for debug: logging to ${PROJECT_BINARY_DIR}/logs/minizip_autobuild_config_debug.log"
            )
            execute_process(
                COMMAND
                    ${CMAKE_COMMAND} -Wno-dev -D CMAKE_CXX_COMPILER=${CMAKE_CXX_COMPILER} -D
                    CMAKE_C_COMPILER=${CMAKE_C_COMPILER} -D CMAKE_LINKER=${CMAKE_LINKER} -D
            ...
            message(
                STATUS
                    "Building minizip Autobuild for debug: logging to ${PROJECT_BINARY_DIR}/logs/minizip_autobuild_build_debug.log"
            )
            execute_process(
        ...
        if(NOT BUILD_DEBUG_ONLY)
            ...
            message(
                STATUS
                    "Configuring Minizip Autobuild for ${MSVC_RELEASE_BUILD_TYPE}: logging to ${PROJECT_BINARY_DIR}/logs/minizip_autobuild_config_release.log"
            )
            execute_process(
                COMMAND
                    ${CMAKE_COMMAND} -Wno-dev -D CMAKE_CXX_COMPILER=${CMAKE_CXX_COMPILER} -D
            ...
            message(
                STATUS
                    "Building minizip Autobuild for ${MSVC_RELEASE_BUILD_TYPE}: logging to ${PROJECT_BINARY_DIR}/logs/minizip_autobuild_build_release.log"
            )
            execute_process(
        ...
        message(
            STATUS
                "Configuring Minizip Autobuild for ${LOCAL_BUILD_TYPE}: logging to ${PROJECT_BINARY_DIR}/logs/minizip_autobuild_config.log"
        )
        execute_process(
            COMMAND
                ${CMAKE_COMMAND} -Wno-dev -D CMAKE_CXX_COMPILER=${CMAKE_CXX_COMPILER} -D
        ...
        message(
            STATUS
                "Building minizip Autobuild for ${CLOCAL_BUILD_TYPE}: logging to ${PROJECT_BINARY_DIR}/logs/minizip_autobuild_build.log"
        )
        execute_process(
