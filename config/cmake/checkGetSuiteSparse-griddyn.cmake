...
macro(SuiteSparse_unzip whichZipFile)
    ...
    if(${resVar} MATCHES "0")
        message(STATUS "SuiteSparse just unzipped in the source dir") # OK
        set(SUITESPARSE_DL_LAST OFF CACHE BOOL "get the last version of suiteSparse" FORCE)
    else()
        message(
            WARNING "something wrong with cmake tar command, redo or try to unzip by yourself..."
        )
...
if(NOT EXISTS "${PROJECT_SOURCE_DIR}/SuiteSparse/CHOLMOD/Include/cholmod.h" AND NOT
                                                                                SUITESPARSE_DL_LAST
)
    message(
        STATUS
            "Apparently, SuiteSparse project is not yet download and extract here. Let me try to do it for you..."
    )
...
if(SUITESPARSE_DL_LAST)
    # JL says: It seems the "current" symlink in the server is no longer available... we'll have to
    # update this manually :-(
    set(SUITESPARSE_URL "http://faculty.cse.tamu.edu/davis/SuiteSparse/SuiteSparse-4.5.6.tar.gz")
    set(SUITESPARSE_ZIP "${PROJECT_SOURCE_DIR}/SuiteSparse.tar.gz")
    if(SUITESPARSE_URL AND SUITESPARSE_ZIP)
        ...
        file(
            DOWNLOAD "${SUITESPARSE_URL}" "${SUITESPARSE_ZIP}"
            INACTIVITY_TIMEOUT 15
            STATUS status
            SHOW_PROGRESS
        )
