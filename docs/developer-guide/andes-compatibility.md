...
GridDyn's current DYR reader recognizes `GENCLS`, `GENROU`, `ESDC1A`, `ESST3A`,
`EXST1`, `EXAC1`, `EXAC2`, `EXAC4`, `EXDC2`, `TGOV1`, `HYGOV`, `IEEEG1`, `IEEEST`,
`ST2CUT`, and `SEXS`. Recognition alone is not compatibility: some adapters
still have attachment, parameter, equation, and validation gaps.

| PSS/e DYR record(s)                            | ANDES destination       | Existing GridDyn candidate                                                    | Current compatibility and required action                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ---------------------------------------------- | ----------------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GENCLS`                                       | `GENCLS`                | `GenModelClassical`                                                           | **Implemented.** The canonical classical model now follows the ANDES/PSS/E equations, with constant emf behind RAW `ZR`/`ZX`, DYR `H`/`D`, copper-loss-aware initialization, and OpenIPSL-compatible `H=0` infinite-bus behavior. Factory, initialization, perturbed-equation, residual, derivative, Jacobian, parameter-validation, IEEE 14 RAW/DYR, and actual ANDES Kundur DYR attachment tests are present. A captured end-to-end disturbed ANDES trajectory remains desirable. |
...
| --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------- |
| `GENCLS`                                                                                                              | `GenModelClassical`; ANDES/PSS/E equations, DYR mapping, initialization, Jacobian, zero-inertia behavior, and IEEE 14 plus Kundur attachment tests are implemented. Add a captured disturbed ANDES trajectory for final cross-solver validation. | Implemented         |
...
| `HYGOV`                                                                                                               | `GovernorHygov`; standard PSS/e/OpenIPSL equations, DYR mapping, initialization, limits, and Jacobian checks are implemented. ANDES's omitted lead path prevents full equation parity.                                                           | Implemented         |
| `TG2`, `TGOV1DB`, `TGOV1N`, `TGOV1NDB`, `IEESGO`, `GAST`, `HYGOVDB`, `HYGOV4`                                         | Existing hydro/reheat/steam classes are candidates only; each needs an equation audit or a new model.                                                                                                                                            | Planned             |
...
24 model families. The current DYR reader recognizes 829 records (`GENROU`,
`IEEEST`, `EXAC2`, `IEEEG1`, `ESDC1A`, `EXAC1`, and `HYGOV`); 1,876 records require
additional support.
...
`EXPIC1` (86), `GENSAL` (22), `IEEET1` (19), `USRMDL` (15),
...
families. The current DYR dispatch recognizes 23,298 records in `GENROU`,
`IEEEST`, `IEEEG1`, `ESDC1A`, `EXAC1`, `EXAC2`, and `HYGOV`; 17,120 records remain
unsupported. The highest-demand missing models are `GGOV1` (3,419), `ESST4B`
(3,187), `GENSAL` (2,306), `IEEET1` (1,907), and `SCRX`
