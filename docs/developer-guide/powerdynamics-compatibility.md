...
| `PSSE_GENSAL`            | No exact salient-pole quadratic-saturation machine   | **P0**   | Has an OpenIPSL test path. Add a distinct salient-pole dq machine and compare saturation, flux states, current, angle, speed, and bases.                                                                                     |
...
| `PSSE_ESST4B`                                         | No exact static exciter                                   | **P0**            | OpenIPSL-tested model. It is distinct from GridDyn's existing `ExciterESST3A`.                                                                                                                                         |
| `PSSE_GGOV1_EXPERIMENTAL`                             | No exact gas/engine governor                              | **P2**            | This is a valuable specification/prototype for `GGOV1`, but PowerDynamics marks it experimental and its test documents known OpenIPSL/reference issues. Do an independent equation and reference audit before porting. |
...
1. Use PowerDynamics' OpenIPSL-tested source and test structures to create GridDyn P0 references for `GENROE`, `GENSAL`, `GENSAE`, `SCRX`, `ESST1A`, and `ESST4B`.
...
5. Treat PowerDynamics' `GGOV1` as a design aid, not validation evidence, until its documented reference issues are independently resolved.
