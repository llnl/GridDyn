                     ...
                     stringVec* warnings)
{
    ...
    for (index_t index = 1; index <= busCount; ++index) {
        ...
        if (bus == nullptr) {
            warning(warnings,
                    "bus slot " + std::to_string(index) + " is not an AC bus and was omitted");
            continue;
        }
        ...
        if ((busNumber == 0) || busNumbers.contains(bus)) {
            ...
            warning(warnings,
                    bus->getName() + " has no usable unique bus ID; assigned " +
    ...
    if (functionName.empty() || std::isdigit(static_cast<unsigned char>(functionName.front()))) {
    ...
    for (const auto* bus : buses) {
        ...
        for (index_t loadIndex = 0; loadIndex < loadCount; ++loadIndex) {
            ...
            if (const auto* zip = dynamic_cast<const ZipLoad*>(load)) {
                ...
                if (std::abs(zip->get("ip", units::MW)) > 1e-12 ||
                    ...
                    warning(warnings,
                            zip->getName() + " has constant-current load terms; they were omitted");
                }
            } else {
                warning(warnings,
                        load->getName() +
        ...
        if (type == 1) {
            warning(warnings, bus->getName() + " is angle-fixed; exported as a PQ bus");
            type = 1;
        } else if (type == 0) {
            type = 1;
        } else if (type == 2) {
            ...
            warning(warnings, bus->getName() + " has an unsupported bus type; exported as PQ");
            type = 1;
        }
        row(output,
        ...
        for (index_t genIndex = 0; genIndex < genCount; ++genIndex) {
            ...
            row(output,
                {static_cast<double>(busNumbers.at(bus)),
                 ...
                 -gen->get("q") * basePower,
                 finiteLimit(
                     gen->get("qmax", units::MVAR), 1.0e6, warnings, gen->getName() + ".qmax"),
                 finiteLimit(
                     gen->get("qmin", units::MVAR), -1.0e6, warnings, gen->getName() + ".qmin"),
                 (vtarget > 0.0) ? vtarget : bus->get("voltage"),
                 basePower,
                 gen->isEnabled() ? 1.0 : 0.0,
                 finiteLimit(
                     gen->get("pmax", units::MW), 1.0e6, warnings, gen->getName() + ".pmax"),
                 finiteLimit(
                     gen->get("pmin", units::MW), -1.0e6, warnings, gen->getName() + ".pmin")});
        }
    }
    output << "    ])\n    ppc[\"branch\"] = array([\n";
    ...
    for (index_t linkIndex = 1; linkIndex <= linkCount; ++linkIndex) {
        ...
        if (acLine == nullptr || bus1 == nullptr || bus2 == nullptr || !busNumbers.contains(bus1) ||
            !busNumbers.contains(bus2)) {
            warning(warnings,
                    link->getName() +
        ...
        row(output,
            {static_cast<double>(busNumbers.at(bus1)),
             static_cast<double>(busNumbers.at(bus2)),
             link->get("r"),
             link->get("x"),
             link->get("b"),
             (ratingA > 1.0e12) ? 0.0 : ratingA,
             0.0,
             0.0,
             link->get("tap"),
             link->get("tapangle", units::deg),
             link->isConnected() ? 1.0 : 0.0,
             -360.0,
             360.0});
    }
    output << "    ])\n    return ppc\n";
