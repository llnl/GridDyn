                                            ...
                                            IOdata& fieldSet)
{
    ...
    gs[1] = saturationFeedback(gs[0]);  // Vr
    gs[2] = gs[0] * Kf / Tf;  // Rf
    if (Tr > 0.0) {
        gs[3] = inputs[VOLTAGE_IN_LOCATION];
    }
                                ...
                                const SolverMode& sMode)
{
    ...
    const auto locations = offsets.getLocations(sD, resid, sMode, this);
...
void ExciterIEEEtype1::derivative(const IOdata& inputs,
                                  ...
                                  const SolverMode& sMode)
{
    auto locations = offsets.getLocations(sD, deriv, sMode, this);
                                        ...
                                        const SolverMode& sMode)
{
    ...
    md.assign(offset, offset, -saturationDerivative(sD.state[offset]) / Te - sD.cj);
    md.assign(offset, offset + 1, 1.0 / Te);
    if (opFlags[OUTSIDE_VOLTAGE_LIMITS]) {
        ...
        if (Tr > 0.0) {
            md.assign(offset + 1, offset + 3, -Ka / Ta);
        } else {
    ...
    if (Tr > 0.0) {
        md.assign(offset + 3, offset + 3, -1.0 / Tr - sD.cj);
        md.assignCheckCol(offset + 3, inputLocs[VOLTAGE_IN_LOCATION], 1.0 / Tr);
    }
}
                                ...
                                const SolverMode& sMode)
{
    ...
    if (opFlags[OUTSIDE_VOLTAGE_LIMITS]) {
        roots[rootOffset] = es[2] - es[0] * Kf / Tf + (Vref + vBias - measuredVoltage(inputs, es)) -
            es[1] / Ka + 0.001 * es[1] / Ka / Ta;
    } else {
        roots[rootOffset] = std::min(regulatorUpperLimit() - es[1], es[1] - Vrmin) + 0.00001;
        if (es[1] >= regulatorUpperLimit()) {
                                       ...
                                       CheckLevel /*level*/)
{
    ...
    if (opFlags[OUTSIDE_VOLTAGE_LIMITS]) {
        double test =
            es[2] - es[0] * Kf / Tf + (Vref + vBias - measuredVoltage(inputs, es)) - es[1] / Ka;
        ...
        if (es[1] > regulatorUpperLimit() + 0.00001) {
