...
{
    constexpr double lowCurrentSlope = 0.577;
    constexpr double highCurrentSlope = 1.732;
    ...
    const double id = inputs[exciterIdInLocation];
    const double iq = inputs[exciterIqInLocation];
    const double vd = inputs[exciterVdInLocation];
    const double vq = inputs[exciterVqInLocation];
    const double realPart = -kpcReal * vd - kpcImag * vq + impedanceImag * id - impedanceReal * iq;
    const double imaginaryPart =
        -kpcImag * vd + kpcReal * vq - impedanceReal * id - impedanceImag * iq;
