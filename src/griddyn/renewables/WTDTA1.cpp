...
void WTDTA1::dynObjectInitializeA(CoreTime time0, std::uint32_t /*flags*/)
{
    if (!std::isfinite(H) || !std::isfinite(DAMP) || !std::isfinite(Htfrac) ||
        !std::isfinite(Freq1) || !std::isfinite(Dshaft) || !std::isfinite(w0) || H <= 0 ||
        Htfrac <= 0 || Htfrac >= 1 || Freq1 <= 0 || Dshaft < 0 || DAMP < 0 || w0 <= 0) {
...
void WTDTA1::derivative(const IOdata& inputs,
...
void WTDTA1::residual(const IOdata& inputs,
                      const StateData& stateData,
                      double resid[],
                      const SolverMode& sMode)
{
...
void WTDTA1::jacobianElements(const IOdata& inputs,
                              const StateData& stateData,
...
stringVec WTDTA1::localStateNames() const
{
    return {"wg", "wt", "Tshaft"};
}
