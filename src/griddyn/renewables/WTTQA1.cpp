...
void WTTQA1::dynObjectInitializeA(CoreTime time0, std::uint32_t /*flags*/)
{
    if ((Tflag != 0 && Tflag != 1) || !std::isfinite(Kpp) || !std::isfinite(Kip) ||
        ...
        !std::isfinite(Temin) || !std::isfinite(TRATE) || Kpp < 0 || Kip < 0 || Tp <= 0 ||
        Twref <= 0 || Temax <= Temin || TRATE <= 0) {
    ...
    for (index_t index = 1; index < 4; ++index) {
        if (electricalPower <= power[index]) {
            return speed[index - 1] +
                ((electricalPower - power[index - 1]) * (speed[index] - speed[index - 1]) /
...
void WTTQA1::dynObjectInitializeB(const IOdata& inputs,
                                  ...
                                  IOdata& fieldSet)
{
    if (inputs.size() < 2 || !std::isfinite(inputs[0]) || !std::isfinite(inputs[1]) ||
        inputs[1] <= 0) {
...
void WTTQA1::derivative(const IOdata& inputs,
...
void WTTQA1::residual(const IOdata& inputs,
                      const StateData& stateData,
                      double resid[],
                      const SolverMode& sMode)
{
...
void WTTQA1::jacobianElements(const IOdata& inputs,
                              const StateData& stateData,
                              ...
                              const SolverMode& sMode)
{
    ...
    if (hasDifferential(sMode)) {
        for (index_t column = 0; column < 3; ++column) {
            std::array<double, 3> plus{loc.diffStateLoc[0],
                                       loc.diffStateLoc[1],
                                       loc.diffStateLoc[2]};
...
double WTTQA1::getOutput(const IOdata& inputs,
                         const StateData& stateData,
                         const SolverMode& sMode,
