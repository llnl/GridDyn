    ...
    if (state.compare_exchange_strong(exp, connection_state_t::operating)) {
        ...
        if (!triggerhalt) {
            socket_.async_receive(asio::buffer(data.data() + residBufferSize,
                                  ...
                                  [ptr = shared_from_this()](const error_type& err, size_t bytes) {
...
void TcpConnection::setErrorCall(
    std::function<bool(TcpConnection::pointer, const error_type&)> errorFunc)
{
    if (state.load() == connection_state_t::prestart) {
...
void TcpConnection::handle_read(const error_type& error, size_t bytes_transferred)
{
