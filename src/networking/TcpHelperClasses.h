...
class TcpConnection: public std::enable_shared_from_this<TcpConnection> {
  public:
    ...
    void setErrorCall(std::function<bool(TcpConnection::pointer, const error_type&)> errorFunc);
...
*/
    void async_receive(std::function<void(TcpConnection::pointer,
                                          const char* buffer,
                                          size_t dataLength,
                                          const error_type& error)> callback)
    {
        socket_.async_receive(asio::buffer(data, data.size()),
                              [connection = shared_from_this(),
                               callback = std::move(callback)](const error_type& error,
...
*/
  ...
  private:
    ...
    void handle_read(const error_type& error, size_t bytes_transferred);
    void handle_read(
        size_t message_size,
        const error_type& error,
        std::function<void(TcpConnection::pointer, const char*, size_t, const error_type& error)>
            callback)
    {
    ...
    std::function<bool(TcpConnection::pointer, const error_type&)> errorCall;
    ...
    const int idcode;
    void connect_handler(const error_type& error);
};  // namespace asio=boost:classTcpConnection:publicstd::enable_shared_from_this<TcpConnection>
...
class TcpAcceptor: public std::enable_shared_from_this<TcpAcceptor> {
  public:
    ...
    void setErrorCall(std::function<bool(TcpAcceptor::pointer, const error_type&)> errorFunc)
    {
  ...
  private:
    ...
    std::function<bool(TcpAcceptor::pointer, const error_type&)> errorCall;
...
@return true if the start up was successful*/
  ...
  private:
    ...
    size_t bufferSize;
    std::function<size_t(TcpConnection::pointer, const char*, size_t)> dataCall;
    std::function<bool(TcpConnection::pointer, const error_type& error)> errorCall;
