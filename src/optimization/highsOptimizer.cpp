...
bool setHighsOptions(Highs& highs, double tolerance, count_t maxIterations)
{
    ...
    if (maxIterations > 0) {
        ...
        if (highs.setOptionValue("simplex_iteration_limit", iterationLimit) ==
                HighsStatus::kError ||
            highs.setOptionValue("ipm_iteration_limit", iterationLimit) == HighsStatus::kError) {
            return false;
        }
    }
    return true;
...
HighsScalingData makeScalingData(const NativeQpProblem& problem, bool enabled)
{
    ...
    for (std::size_t row = 0; row < problem.constraintCount; ++row) {
        for (std::size_t entry = problem.constraintMatrix.rowStarts[row];
            ...
            columnMagnitude[column] =
                (std::max)(columnMagnitude[column], std::abs(problem.constraintMatrix.values[entry]));
        }
    }
    ...
    for (std::size_t row = 0; row < problem.constraintCount; ++row) {
        for (std::size_t entry = problem.constraintMatrix.rowStarts[row];
             ...
             ++entry) {
            const auto column = problem.constraintMatrix.columnIndices[entry];
            rowMagnitude[row] = (std::max)(
                rowMagnitude[row],
                std::abs(problem.constraintMatrix.values[entry] * scaling.variableScale[column]));
        }
...
HighsModel makeHighsModel(const NativeQpProblem& problem, const HighsScalingData& scaling)
{
    ...
    for (std::size_t column = 0; column < problem.variableCount; ++column) {
        ...
        lp.col_lower_[column] = scaleFiniteBound(problem.variableLowerBounds[column], variableScale);
        lp.col_upper_[column] = scaleFiniteBound(problem.variableUpperBounds[column], variableScale);
    }
    ...
    if (hasQuadraticTerm) {
        ...
        for (std::size_t column = 0; column < problem.variableCount; ++column) {
            ...
            const double variableScale = scaling.variableScale[column];
            hessian.value_.push_back(
                2.0 * problem.quadraticObjective[column] * variableScale * variableScale);
        }
        hessian.start_.push_back(static_cast<HighsInt>(hessian.value_.size()));
    }
    return model;
...
int HighsOptimizer::solve(double tStop, double& tReturn)
{
    ...
    Highs highs;
    const double tolerance = (std::isfinite(rtol) && (rtol > 0.0)) ?
        (std::max)(rtol, 1e-10) :
        1e-8;
    ...
    const auto qpNullspaceLimit = static_cast<HighsInt>(
        (std::max)(problem().variableCount, static_cast<std::size_t>(4000)));
