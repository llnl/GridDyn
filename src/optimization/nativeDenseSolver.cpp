...
bool isEquality(double lower, double upper, double tolerance)
{
    return std::isfinite(lower) && std::isfinite(upper) &&
        std::abs(lower - upper) <= tolerance * (1.0 + (std::max)(std::abs(lower), std::abs(upper)));
}
                                  ...
                                  std::vector<double>& variableScale)
{
    ...
    for (std::size_t column = 0; column < problem.variableCount; ++column) {
        ...
        scaled.quadraticObjective[column] =
            problem.quadraticObjective[column] * scale * scale;
    }
    ...
    for (std::size_t row = 0; row < problem.constraintCount; ++row) {
        ...
        for (std::size_t column = 0; column < problem.variableCount; ++column) {
            const double coefficient = problem.constraintMatrix[rowStart + column] *
                variableScale[column];
        ...
        for (std::size_t column = 0; column < problem.variableCount; ++column) {
            initialValue += scaled.constraintMatrix[rowStart + column] *
                                        ...
                                        const NativeDenseSolverOptions& options)
{
    ...
    for (std::size_t row = 0; row < full.constraintCount; ++row) {
        ...
        if (reduced.constraintLowerBounds[row] > reduced.constraintUpperBounds[row] +
                options.feasibilityTolerance *
                    (1.0 + (std::max)(std::abs(reduced.constraintLowerBounds[row]),
        ...
        for (std::size_t reducedColumn = 0;
             reducedColumn < reduced.variableCount;
             ++reducedColumn) {
                                         ...
                                         const NativeDenseSolverOptions& options)
{
    ...
    for (std::size_t row = 0; row < problem.constraintCount; ++row) {
        ...
        const auto rowStart = row * problem.variableCount;
        std::copy_n(problem.constraintMatrix.begin() +
    ...
    for (std::size_t row = 0; row < problem.constraintCount; ++row) {
        ...
        const auto rowStart = row * problem.variableCount;
        reduced.constraintMatrix.insert(
            reduced.constraintMatrix.end(),
            problem.constraintMatrix.begin() + static_cast<std::ptrdiff_t>(rowStart),
            problem.constraintMatrix.begin() +
                static_cast<std::ptrdiff_t>(rowStart + problem.variableCount));
    }
    transform.problem = std::move(reduced);
    return {};
}
    ...
    for (std::size_t column = 0; column < problem.variableCount; ++column) {
        if (std::isfinite(problem.variableLowerBounds[column])) {
            violation = (std::max)(violation,
                                   problem.variableLowerBounds[column] - values[column]);
        }
        if (std::isfinite(problem.variableUpperBounds[column])) {
            violation = (std::max)(violation,
                                   values[column] - problem.variableUpperBounds[column]);
        }
    }
    return (std::max)(0.0, violation);
}
                           ...
                           const std::vector<double>& multipliers)
{
    ...
    for (std::size_t active = 0; active < workingSet.size(); ++active) {
        const auto& coefficients = model.constraints[static_cast<std::size_t>(workingSet[active])].coefficients;
        for (std::size_t column = 0; column < model.variableCount; ++column) {
                              ...
                              const std::vector<double>& multipliers)
{
    double maximum = 0.0;
    for (std::size_t active = 0; active < workingSet.size(); ++active) {
        const auto& constraint = model.constraints[static_cast<std::size_t>(workingSet[active])];
        if (constraint.type == ConstraintType::EQUALITY) {
            continue;
        }
        const double slack = (std::max)(0.0, constraint.rhs - constraintValue(constraint, values));
                                 ...
                                 const NativeDenseSolverOptions& options)
{
    ...
    for (std::size_t iteration = 0; iteration < options.maxIterations; ++iteration) {
        ...
        for (std::size_t active = 0; active < activeCount; ++active) {
            ...
            for (std::size_t column = 0; column < model.variableCount; ++column) {
                kktMatrix[column * kktDimension + model.variableCount + active] = coefficients[column];
                kktMatrix[(model.variableCount + active) * kktDimension + column] = coefficients[column];
            }
        }

        const auto kktResult = solveLinearSystem(std::move(kktMatrix),
                                                 std::move(kktRhs),
        ...
        if (kktResult.singular) {
            ...
            for (auto candidate = workingSet.end(); candidate != workingSet.begin();) {
                --candidate;
                const auto& constraint = model.constraints[static_cast<std::size_t>(*candidate)];
        ...
        if (stepNorm <= stepTolerance) {
            ...
            result.maximumConstraintViolation = maximumConstraintViolation(model, result.values);
        ...
        for (std::size_t index = 0; index < model.constraints.size(); ++index) {
            ...
            if (candidateStep < -options.feasibilityTolerance) {
                result.status = NativeSolveStatus::NUMERICAL_FAILURE;
                result.message = "active-set step encountered an infeasible blocking constraint";
                result.activeSetSize = workingSet.size();
                return result;
            }
            ...
            const double stepTieTolerance = options.feasibilityTolerance *
                (std::max)(std::abs(stepLength), 1e-12);
            if (candidateStep < stepLength - stepTieTolerance ||
                ((std::abs(candidateStep - stepLength) <= stepTieTolerance) &&
                 ((blockingConstraint < 0) || (static_cast<int>(index) < blockingConstraint)))) {
        ...
        if (blockingConstraint < 0) {
            ...
            for (std::size_t column = 0; column < model.variableCount; ++column) {
                curvature += 2.0 * model.quadraticObjective[column] *
    ...
    for (std::size_t row = 0; row < problem.constraintCount; ++row) {
        ...
        if (isEquality(lower, upper, feasibilityTolerance)) {
            ...
            if (std::abs(equalityResidual) <= feasibilityTolerance) {
                ...
                phase.sides.push_back({std::move(coefficients),
                                       -lower,
                                       -equalityResidual});
                ...
                phase.sides.push_back({baseCoefficients,
                                       upper,
                                       equalityResidual});
            }
            continue;
        ...
        if (isFiniteLower(lower) &&
            (lower - rowValue > feasibilityTolerance)) {
            auto coefficients = baseCoefficients;
            for (auto& coefficient : coefficients) {
                coefficient = -coefficient;
            }
            ...
            phase.model.constraints.push_back(
                {std::move(lowerCoefficients),
                 -lower,
                  ConstraintType::INEQUALITY});
        }
        if (isFiniteUpper(upper) &&
            (rowValue - upper > feasibilityTolerance)) {
            phase.sides.push_back({baseCoefficients,
                                   upper,
                                   rowValue - upper});
                              ...
                              const NativeDenseSolverOptions& options)
{
    ...
    if (result.status != NativeSolveStatus::OPTIMAL) {
        result.status = (result.status == NativeSolveStatus::SINGULAR) ?
            NativeSolveStatus::NUMERICAL_FAILURE : result.status;
                      ...
                      NativeSolveStatus& status)
{
    ...
    if ((options.maxIterations == 0) ||
        (options.feasibilityTolerance <= 0.0) ||
        (options.optimalityTolerance <= 0.0) ||
        (options.pivotTolerance <= 0.0)) {
        status = NativeSolveStatus::NUMERICAL_FAILURE;
