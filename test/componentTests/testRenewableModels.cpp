...
void loadOtherMachines(GridDynSimulation& simulation)
{
    const auto source=std::filesystem::path(__FILE__).parent_path().parent_path() /
        "test_files" / "comparison_tests" / "ieee14_gencls.dyr";
    const auto file=std::filesystem::temp_directory_path() /
...
TEST(RenewableModels, FactoryAndRoleReplacement)
{
    ...
    std::unique_ptr<CoreObject> hostObject(
        factory->createObject("generator", "renewable_dynamic"));
    std::unique_ptr<CoreObject> modelObject(
        factory->createObject("renewable_model", "regca1"));
...
TEST(RenewableModels, REGCA1SteadyInitializationAndHostSign)
{
    ...
    double residual[5]{};
    converter->residual({1.0, kNullVal, kNullVal}, emptyStateData,
                        residual, cLocalSolverMode);
...
TEST(RenewableModels, REGCA1ZeroReactiveRecoveryLimitsAreDirectional)
{
    ...
    converter.derivative({1.0, 0.8, -0.4}, emptyStateData,
                         rate.data(), cLocalSolverMode);
...
TEST(RenewableModels, REECA1ZeroCurvesAndZeroOrderingLag)
{
    ...
    EXPECT_NEAR(controller.getOutputs({1.0}, emptyStateData,
                                      cLocalSolverMode)[2], 0.8, 1e-12);
    EXPECT_DOUBLE_EQ(controller.get("vq4"),0.0);
    double update[6]{};
    EXPECT_THROW(controller.algebraicUpdate({0.7},emptyStateData,update,
...
TEST(RenewableModels, ACTIVSg25kDyrProfileLoadsRecoveryHold)
{
    ...
    std::filesystem::remove(file);
    auto* generator=dynamic_cast<RenewableGenerator*>(
        simulation->getBus(0)->getGen(0));
...
TEST(RenewableModels, REECA1HoldsDipLimitAfterRecovery)
{
    ...
    const double heldCurrent=controller.getOutputs({1.0},emptyStateData,
                                                    cLocalSolverMode)[0];
    ...
    EXPECT_GT(controller.getOutputs({1.0},emptyStateData,
                                    cLocalSolverMode)[0],heldCurrent+0.02);
}
...
TEST(RenewableModels, REGCA1VoltageStepAndMachineBase)
{
    ...
    EXPECT_NEAR(converter->getStates()[0], 0.8, 1e-12);
    EXPECT_NEAR(host.getOutputs({1.0, 0.0}, emptyStateData,
                                cLocalSolverMode)[0], -0.4, 1e-12);
    ...
    EXPECT_NEAR(host.getOutputs({0.6, 0.0}, emptyStateData,
                                cLocalSolverMode)[0], -0.12, 1e-12);
}
...
TEST(RenewableModels, REGCA1DaeJacobianMatchesResidual)
{
    ...
    converter.jacobianElements(inputs, stateData, jacobian,
                               {6, kNullLocation, kNullLocation}, cDaeSolverMode);
    ...
    for (index_t column = 0; column < state.size(); ++column) {
        ...
        for (index_t row = 0; row < state.size(); ++row) {
            EXPECT_NEAR(jacobian.at(row, column),
                        (residual[row] - base[row]) / step, 1e-5)
...
TEST(RenewableModels, DyrAssemblesIndependentModelsInEitherOrder)
{
    for (const std::vector<std::string_view>& order :
        ...
        const auto output = generator->getOutputs({1.0, 0.0}, emptyStateData,
                                                   cLocalSolverMode);
        EXPECT_NEAR(output[0], -0.8, 1e-12);
        EXPECT_NEAR(output[1], -0.1, 1e-12);
    }
...
TEST(RenewableModels, DyrRejectsDuplicateAndUnsupportedRecords)
{
    ...
    EXPECT_THROW(loadRenewableRecords(*duplicate, {"REGCA1", "REGCA1"}),
                 InvalidParameterValue);
...
TEST(RenewableModels, ThreeModelDaeJacobianIncludesSignalConnections)
{
    ...
    for (index_t column = 0; column < state.size(); ++column) {
        ...
        for (index_t row = 0; row < state.size(); ++row) {
            EXPECT_NEAR(jacobian.at(row, column),
                        (residual[row] - base[row]) / step, 2e-4)
                << "row " << row << " column " << column;
        }
    }
}
...
TEST(RenewableModels, WindDyrAssemblesConnectedChainInEitherOrder)
{
    for (const std::vector<std::string_view>& order :
        ...
        auto* host=dynamic_cast<RenewableGenerator*>(bus->getGen(0));
        ASSERT_NE(host,nullptr);
        auto* drivetrain=dynamic_cast<WTDTA1*>(host->getSubObject(
            "renewable_component",static_cast<index_t>(RenewableRole::driveTrain)));
        auto* aero=dynamic_cast<WTARA1*>(host->getSubObject(
            "renewable_component",static_cast<index_t>(RenewableRole::aerodynamics)));
        auto* pitch=dynamic_cast<WTPTA1*>(host->getSubObject(
            "renewable_component",static_cast<index_t>(RenewableRole::pitchControl)));
        auto* torque=dynamic_cast<WTTQA1*>(host->getSubObject(
            "renewable_component",static_cast<index_t>(RenewableRole::torqueControl)));
...
{
    const auto raw=std::filesystem::path(__FILE__).parent_path().parent_path() /
        "test_files" / "comparison_tests" / "ieee14.raw";
    auto simulation=std::make_unique<GridDynSimulation>();
    loadFile(simulation.get(),raw.string());
    loadRenewableRecords(*simulation,{"REGCA1","REECA1","WTDTA1",
                                      "WTARA1","WTPTA1","WTTQA1"},2);
...
TEST(RenewableModels, TwoBusRenewableFaultMatchesAndesReference)
{
  for (int profile=0;profile<4;++profile) {
    ...
    SCOPED_TRACE(includeWind ? "wind assembly" :
                 includePlant ? "REPCA1" :
                 useReecb ? "REECB1" : "REGCA1+REECA1");
    const auto xml=std::filesystem::path(__FILE__).parent_path().parent_path() /
        "reference" / "renewable_fault" / "two_bus.xml";
    auto simulation=std::make_unique<GridDynSimulation>();
    ...
    const std::string header="time,voltage,converter_p,converter_q,electrical_ip,electrical_iq";
    EXPECT_EQ(line,includeWind ? header+
